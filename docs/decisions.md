...
No hay ninguna API hotelera con precios y disponibilidad accesible gratis y sin contrato (Booking y Expedia exigen ser partner; Amadeus Self-Service cerró el 17-07-2026). Se construye el Modo A. El Modo B queda como interfaz de adaptadores **sin implementar y sin simulaciones**. Evidencia: `provider-research.md` §2. **Estado: pendiente de tu aprobación.**

## D-002 · 2026-09-23 · Proxy en Cloudflare para Nominatim y Overpass
El navegador no llamará directamente a Nominatim ni a Overpass. Motivos: la política de Nominatim cuenta el tráfico total de la app, exige identificación (`User-Agent`, imposible de fijar desde el navegador) y recomienda proxy con caché; Overpass admite ~100 consultas/día en uso regular. Sin autocompletado (búsqueda al pulsar «Buscar»). Límite: la caché de Cloudflare es por centro de datos; no se promete límite global exacto. Evidencia: `provider-research.md` §6. **Estado: pendiente de aprobación.**
...
- Se revisa la decisión cuando pruebes los enlaces en tu navegador (Fase 3). **Estado: pendiente de aprobación.**
