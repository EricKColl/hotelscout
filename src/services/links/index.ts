...
export function buildBookingSearchUrl(destinationText: string, trip: Trip): string | undefined {
...
export function buildLinksForLodging(subject: LinkSubject, trip: Trip | undefined): ExternalLink[] {
  ...
  if (trip) {
    const url = buildBookingSearchUrl([subject.name, subject.city].filter(Boolean).join(', '), trip)
    if (url) {
      links.push({
        kind: 'search',
        label: 'Buscar en Booking.com',
        url,
        note: 'Abre una búsqueda con tus fechas; el precio y la disponibilidad se comprueban allí.',
      })
    }
  }
  links.push({
